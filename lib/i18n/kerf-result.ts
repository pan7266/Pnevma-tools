...
export function buildLocalizedLightBurnNotes(result: KerfAdvisorResult, inputs: KerfAdvisorInputs, labels: Labels): string {
  ...
  return [
    ...
    `${labels.rayleigh}: ${result.rayleighRangeMm.toFixed(4)} mm`,
    `${labels.depthOfFocusConfocal}: ${result.confocalParameterMm.toFixed(4)} mm`,
    `${labels.recommendedFocusDepth}: ${result.recommendedFocusDepthMm.toFixed(3)} mm (${result.recommendedFocusPercent.toFixed(1)}%)`,
    `${labels.acceptableFocusRange}: ${result.acceptableFocusMinMm.toFixed(3)} - ${result.acceptableFocusMaxMm.toFixed(3)} mm`,
    `${labels.placement}: ${labels[result.placementLabelKey] || result.placementLabelKey}`,
    `${labels.measuredKerf}: ${measuredKerf ? `${measuredKerf.toFixed(4)} mm` : labels.notMeasured}`,
    ...
    `${labels.externalOffset}: ${externalOffset ? `${externalOffset.toFixed(4)} mm ${labels.outward}` : labels.measureKerfFirst}`,
    `${labels.internalOffset}: ${internalOffset ? `${internalOffset.toFixed(4)} mm ${labels.inward}` : labels.measureKerfFirst}`,
    labels.slotTabNote,
