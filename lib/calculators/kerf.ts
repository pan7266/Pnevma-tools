...
export function calculateKerfAdvisor(inputs: KerfAdvisorInputs): KerfAdvisorResult {
  ...
  const lightBurnNotes = [
    ...
    `Rayleigh range: ${beam.zR.toFixed(4)} mm`,
    `Depth of focus / confocal: ${beam.confocal.toFixed(4)} mm`,
    `Recommended focus depth: ${rule.focus.toFixed(3)} mm (${percent.toFixed(1)}%)`,
    `Acceptable focus range: ${rule.min.toFixed(3)} to ${rule.max.toFixed(3)} mm`,
    `Placement label: ${rule.placement}`,
    `Measured kerf: ${kerf ? `${kerf.toFixed(4)} mm` : "not measured"}`,
    ...
    `External contour offset: ${externalOffset ? `${externalOffset.toFixed(4)} mm outward` : "measure kerf first"}`,
    `Internal contour offset: ${internalOffset ? `${internalOffset.toFixed(4)} mm inward` : "measure kerf first"}`,
